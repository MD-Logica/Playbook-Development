...
- **Appointment Settings Enhancements:** Customizable appointment statuses (system + custom with color/terminal flags), readiness indicators (with integration type: Manual/Stripe/DocuSign/PandaDoc, drag-to-reorder, enable/disable toggle), internal block types (system + custom with color). All three have full CRUD API routes with lazy seeding of defaults on first GET. Settings pages under Appointments section in sidebar. Schema includes `AppointmentAttendee` join model for multi-provider support (PR-G2b dependency).
