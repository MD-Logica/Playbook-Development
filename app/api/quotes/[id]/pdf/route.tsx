...
function QuoteDocument({ data }: { data: any }) {
  const accentColor = data.practice?.primaryColor || "#1a3a2a";
  const patient = data.patient;
  const coordinator = data.coordinator;
  const practice = data.practice;
  const total = Number(data.total || 0);
  const depositValue = Number(data.depositValue || 0);
  const depositAmount = data.depositType === "PERCENTAGE"
    ...
    createElement(Page, { size: "A4", style: styles.page },
      ...
      createElement(View, { style: styles.headerRow },
        ...
        createElement(View, { style: styles.headerRight },
          createElement(Text, { style: styles.quoteLabel }, "QUOTE"),
          createElement(Text, { style: styles.quoteNumber }, data.quoteNumber ? `#${data.quoteNumber}` : ""),
        ),
      ),
      ...
      createElement(View, { style: styles.infoRow },
        createElement(View, { style: styles.infoCol },
          createElement(Text, { style: styles.infoLabel }, "PREPARED FOR"),
      ...
      ...(data.lineItems || []).flatMap((li: any) => {
        ...
        if (data.showComponents && li.productService?.itemType === "BUNDLE" && li.productService?.bundleComponents?.length) {
          li.productService.bundleComponents.forEach((bc: any, idx: number) => {
            elements.push(
              createElement(View, { style: styles.bundleRow, key: `bundle-${li.id}-${idx}` },
                createElement(Text, { style: styles.bundleText },
                  `• ${bc.item?.name || "Component"} — ${formatCurrency(bc.item?.price)}`
                ),
              )
            );
          });
      ...
      createElement(View, { style: styles.totalsContainer },
        createElement(View, { style: styles.totalsBox },
          ...
          createElement(View, { style: styles.totalRow },
            ...
            createElement(Text, { style: styles.grandTotalValue }, formatCurrency(data.total)),
          ),
        ),
      ),

      depositValue > 0
        ? createElement(View, { style: [styles.depositBox, { borderLeftColor: accentColor }] },
            createElement(Text, { style: styles.depositLabel }, "DEPOSIT TO SECURE YOUR DATE"),
            createElement(Text, { style: [styles.depositAmount, { color: accentColor }] },
              formatCurrency(depositAmount)
            ),
            data.depositType === "PERCENTAGE"
              ? createElement(Text, { style: styles.depositSubtext },
      ...
      createElement(Text, { style: styles.financingText },
        "Financing options available: CareCredit \u2022 Cherry \u2022 PatientFi"
      ),
      ...
      createElement(View, { style: styles.footer },
        data.expirationDate
          ? createElement(Text, { style: styles.footerText },
              `This quote is valid until ${formatDate(data.expirationDate)}`
            )
          : null,
        practice
          ? createElement(Text, { style: styles.footerText },
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const buffer = await renderToBuffer(
      createElement(QuoteDocument, { data: quote }) as any
    );

    const filename = quote.quoteNumber ? `Quote-${quote.quoteNumber}.pdf` : "Quote.pdf";
